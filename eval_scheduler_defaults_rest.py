# eval_scheduler_defaults_rest.py
import asyncio
        ...
        if isinstance(names, list):
            ...
            for n in names:
                if isinstance(n, RestSlot):
                    ...
                    rest_captured = True
                    break  # must be last
                elif isinstance(n, DestructureSlot):
                    v = values[i] if i < len(values) else None
                    if v is None and n.default:
                        v = n.default.accept(self)
                    self.env[n.name] = v
            ...
            # If no rest slot but more values exist → ignore extras
            if not rest_captured and i < len(values):
                pass  # extras are discarded
        elif isinstance(names, DestructureSlot):
            if values is None and names.default:
                val = names.default.accept(self)
            else:
                val = values
            self.env[names.name] = val
